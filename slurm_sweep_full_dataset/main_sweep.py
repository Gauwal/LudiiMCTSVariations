...
def compute_predictor_performance(original_results, true_win_rate, model_folder):
    ...
    try:
        ...
        total_pred = sum(len(preds[g]) if g in preds and isinstance(preds[g], dict) else 0 for g in preds)
...
def run_experiment(method, time_limit, seed, batch_size, alpha, confidence_bound, output_dir):
    ...
    per_method_n = max(batch_size * 2, int(total_pulls // (1 + len(weights))))
    ...
    predictor_regret, predictor_ape = compute_predictor_performance(original_results, true_win_rate, model_folder)
    print()
    
    # Run experiment for specified method
    ...
    if is_baseline:
        ...
        print(f'Baseline finished: {len(regret_base)} points, {base_time:.2f}s wall time')
        ...
        for g in original_results.keys():
            ...
            for a in original_results[g].keys():
                ...
                if p is not None:
                    # Sample Bernoulli(p) pseudo-observations
    ...
    method_str = method.replace(':', '_')  # 'baseline' or 'warm_50'
    ...
    # Plot regret
    if is_baseline:
        all_regrets = [regret_results]
        plot_label = 'Baseline'
    else:
        all_regrets = [regret_results]
        plot_label = f'Warm w={warm_weight}'
    
    min_len = len(regret_results) if len(regret_results) > 0 else 0
    
    if min_len > 0:
        x = [(i+1) * batch_size for i in range(min_len)]
        plt.figure(figsize=(10, 6))
        plt.plot(x, regret_results, label=plot_label, color='blue', linewidth=2)
        
        # Add predictor line if available
        if predictor_regret is not None:
            plt.hlines(predictor_regret, x[0], x[-1], colors='red', linestyles=':', linewidth=2.5, label='Predictor (single)')
    ...
    # Plot APE
    min_len_ape = len(ape_results) if len(ape_results) > 0 else 0
    
    if min_len_ape > 0:
        x_ape = [(i+1) * batch_size for i in range(min_len_ape)]
        plt.figure(figsize=(10, 6))
        plt.plot(x_ape, ape_results, label=plot_label, color='blue', linewidth=2)
        
        # Add predictor line if available
        if predictor_ape is not None:
            plt.hlines(predictor_ape, x_ape[0], x_ape[-1], colors='red', linestyles=':', linewidth=2.5, label='Predictor (single)')
...
def main():
    ...
    parser.add_argument('--method', type=str, required=True,
                        choices=['baseline'] + [f'warm:{w}' for w in DEFAULT_WEIGHTS],
                        help='Method to run: "baseline" or "warm:<weight>"')
